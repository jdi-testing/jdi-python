...
class AssertUtils:
    ...
    def _assert_true(result, assert_name, assert_log):
        if (not callable(result) and not result) or (callable(result) and not result()):
            logger.error(assert_name + ":::" + assert_log + ":::Failed")
            raise AssertionError(assert_name + ":::" + assert_log + ":::Failed")
        else:
            logger.info(str(assert_name) + ":::Success")
...
class Assert:
    ...
    def assert_element_test(el, expected_text, text=None):
        ...
        AssertUtils._assert_true(
            lambda: actual_text == expected_text,
            "Assert element test" if text else text,
    ...
    def assert_contains(actual_text, expected_text, text=None):
        AssertUtils._assert_true(
            lambda: actual_text.find(expected_text) != -1,
    ...
    def wait_assert_equal(function_actual_text, expected_text, seconds=5, text=None):
        AssertUtils._assert_true(
            lambda: AssertUtils._repeat_while_not_true(lambda: function_actual_text() == expected_text, seconds),
            "Wait text equal" if text is None else text,
            "Check text '{0}' equals '{1}' in {2} seconds".format(expected_text, function_actual_text(), seconds),
        )
    ...
    def assert_true(condition, text=None):
        AssertUtils._assert_true(
            condition, "Assert condition is true" if text is None else text, "Check that {0} is true".format(condition)
        )
